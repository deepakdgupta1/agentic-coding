...
acfs_sandbox_exists() {
    lxc info "$ACFS_CONTAINER_NAME" &>/dev/null 2>&1
}

# Check if ACFS container is running
acfs_sandbox_running() {
    local state
    state=$(lxc info "$ACFS_CONTAINER_NAME" 2>/dev/null | grep -E "^Status:" | awk '{print $2}')
    [[ "$state" == "RUNNING" ]]
}
...
_acfs_create_profile() {
    ...
    lxc profile show "$ACFS_PROFILE_NAME" &>/dev/null && \
        lxc profile delete "$ACFS_PROFILE_NAME" 2>/dev/null || true

    # Create profile with workspace bind mount and port forwarding
    lxc profile create "$ACFS_PROFILE_NAME"

    lxc profile set "$ACFS_PROFILE_NAME" security.nesting=true

    # Add workspace disk device
    lxc profile device add "$ACFS_PROFILE_NAME" workspace disk \
        source="$ACFS_WORKSPACE_HOST" \
        path="$ACFS_WORKSPACE_CONTAINER"

    # Add port forwarding for dashboard
    lxc profile device add "$ACFS_PROFILE_NAME" dashboard-proxy proxy \
...
acfs_sandbox_create() {
    ...
    lxc launch "$ACFS_CONTAINER_IMAGE" "$ACFS_CONTAINER_NAME" \
        --profile default \
    ...
    while [[ $retries -gt 0 ]]; do
        if lxc exec "$ACFS_CONTAINER_NAME" -- systemctl is-system-running &>/dev/null 2>&1; then
            break
        fi
        sleep 1
        ((retries--))
    ...
    lxc exec "$ACFS_CONTAINER_NAME" -- bash -c '
...
acfs_sandbox_exec() {
    ...
    if [[ -z "$cmd" ]]; then
        # Interactive shell
        lxc exec "$ACFS_CONTAINER_NAME" -- sudo -u ubuntu -i
    else
        # Execute command
        lxc exec "$ACFS_CONTAINER_NAME" -- sudo -u ubuntu bash -c "$cmd"
    fi
}
...
acfs_sandbox_exec_root() {
    ...
    lxc exec "$ACFS_CONTAINER_NAME" -- bash -c "$cmd"
}
...
acfs_sandbox_destroy() {
    ...
    lxc stop "$ACFS_CONTAINER_NAME" --force 2>/dev/null || true
    lxc delete "$ACFS_CONTAINER_NAME" --force
...
acfs_sandbox_status() {
    ...
    if [[ "$state" == "RUNNING" ]]; then
        ip=$(lxc info "$ACFS_CONTAINER_NAME" 2>/dev/null | grep -A1 "eth0:" | grep "inet:" | awk '{print $2}' | cut -d/ -f1 | head -1)
        echo "IP: ${ip:-unknown}"
